...
class Ckan_Backend_Sync_Local_Organisation extends Ckan_Backend_Sync_Abstract {
	...
	protected function get_ckan_data( $post ) {
		...
		$federal_level = Ckan_Backend_Helper::get_metafield_value( $post->ID, $this->field_prefix . 'federal_level', $load_from_post );
		...
		if ( current_user_can( 'edit_data_of_all_organisations' ) ) {
			...
			if ( '' !== $federal_level ) {
				$data['federal_level'] = $federal_level;
			}
		} else {
			$_POST[ $this->field_prefix . 'parent' ] = get_post_meta( $post->ID, $this->field_prefix . 'parent', true );
			$_POST[ $this->field_prefix . 'federal_level' ] = get_post_meta( $post->ID, $this->field_prefix . 'federal_level', true );
		}
